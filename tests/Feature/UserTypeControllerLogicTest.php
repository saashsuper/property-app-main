...
class UserTypeControllerLogicTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function admin_can_delete_user_type_without_users()
    {
        ...
        // Since UserType uses soft deletes, check that it's soft deleted
        $this->assertSoftDeleted('user_types', ['id' => $userType->id]);
    }
    ...
    public function user_type_visibility_scope_works()
    {
        ...
        // Test the scope directly - should only return visible types
        $visibleTypes = UserType::visible()->get();
        
        // Check that our new visible type is in the results
        $this->assertTrue($visibleTypes->contains($visibleType));
        
        // Check that our new hidden type is NOT in the results
        $this->assertFalse($visibleTypes->contains($hiddenType));
        
        // Check that all returned types are visible
    ...
    /** @test */
    public function user_type_has_users_relationship()
    {
        $userType = UserType::factory()->create();
        $user1 = User::factory()->withUserType($userType->id)->create();
        $user2 = User::factory()->withUserType($userType->id)->create();
    ...
    /** @test */
    public function user_type_with_count_works()
    {
        $userType = UserType::factory()->create();
        User::factory()->withUserType($userType->id)->count(3)->create();
        
        $userTypeWithCount = UserType::withCount('users')->find($userType->id);
        
        $this->assertEquals(3, $userTypeWithCount->users_count);
    }
}
