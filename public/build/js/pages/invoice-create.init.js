...
var paymentSign = "$";
Array.from(document.getElementsByClassName("invoice-product-line-price")).forEach(function (item) {
...
document.querySelector("#logo-img-file-input").addEventListener("change", function () {
    var preview = document.querySelector(".card-logo-image");
...
// sign image
document.querySelector("#sign-img-file-input").addEventListener("change", function () {
    var preview = document.querySelector(".card-sign-image");
    var file = document.querySelector(".sign-img-file-input").files[0];
    var reader = new FileReader();
    reader.addEventListener("load",function () {
            preview.src = reader.result;
        },
        ...
        Array.from(plus).forEach(function (e) {
            e.onclick = function (event) {
                if (parseInt(e.previousElementSibling.value) < 10) {
                    ...
                    var priceselection = e.parentElement.parentElement.nextElementSibling.querySelector(".invoice-product-line-price");
                    var productQty = e.parentElement.querySelector(".invoice-product-quantity").value;
...
function new_link() {
    ...
    var delLink =
        ...
        '<input type="number" class="invoice-product-quantity" id="product-qty-' + count + '" value="0" readonly>' +
        '<button type="button" class="plus-btn">+</button>' +
        "</div>" +
        "</td>" +
        '<td class="text-end">' +
        "<div>" +
        '<input type="text" class="form-control invoice-product-line-price" id="productPrice-' + count + '" value="$0.00" placeholder="$0.00" />' +
        "</div>" +
        "</td>" +
...
function recalculateCart() {
    ...
    Array.from(document.getElementsByClassName("product-elem")).forEach(function (item) {
        Array.from(item.getElementsByClassName("invoice-product-line-price")).forEach(function (e) {
            if (e.value) {
...
function amountKeyup() {
    ...
    Array.from(document.getElementsByClassName('product-price')).forEach(function (item) {
        item.addEventListener('keyup', function (e) {
            ...
            var priceselection = item.parentElement.nextElementSibling.nextElementSibling.querySelector('.invoice-product-line-price');

            var amount = e.target.value;
            var itemQuntity = item.parentElement.nextElementSibling.querySelector('.invoice-product-quantity').value;
...
if (localStorage.getItem("invoice_no") === null && localStorage.getItem("option") === null) {
    viewobj = '';
    var value = "#TBS" + Math.floor(11111111 + Math.random() * 99999999);
...
if ((viewobj != undefined) && (options == "edit-invoice")) {
    ...
    document.getElementById("invoicenoInput").value = "#TBS" + viewobj.invoice_no;
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    formEvent.addEventListener("submit", function (event) {
        ...
        var logoVal = document.querySelector(".card-logo-image").src;
        var duedate_val = document.getElementById("due-date-field").value;
        ...
        var signInputVal = document.querySelector(".card-sign-image").src;
        ...
        var products = document.getElementsByClassName("product-elem");
        var count = 1;
        var new_product_obj = [];
        ...
        if (formEvent.checkValidity() === false) {
            ...
            if ((options == "edit-invoice") && (invoice_no == i_no)) {
                ...
                invoices[objIndex].logo_img = logoVal;
                invoices[objIndex].customer = billing_address_full_name;
                invoices[objIndex].email = email;
                invoices[objIndex].createDate = date;
                invoices[objIndex].dueDate = duedate_val;
                ...
                invoices[objIndex].products = new_product_obj;
                invoices[objIndex].notes = notes;
                invoices[objIndex].sign_img = signInputVal;
                ...
                var new_data_object = {
                    ...
                    products: new_product_obj,
                    notes: notes,
                    sign_img: signInputVal
                };
