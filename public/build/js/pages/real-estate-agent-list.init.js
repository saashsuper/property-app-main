...
xhttp.onload = function () {
    ...
    Array.from(json_records).forEach(function (element) {
        agentList.add({
            agent_id: '<a href="apps-real-estate-agent-overview.html" class="fw-medium link-primary">#TBS' + element.id + '</a>',
            ...
            agent_Name: '<div class="d-flex align-items-center gap-2">\
                ...
                <a href="apps-real-estate-agent-overview.html" class="text-reset text-capitalize">'+ element.agent[0].name + '</a>\
            </div>',
    ...
    agentList.remove("agent_id", `<a href="apps-real-estate-agent-overview.html" class="fw-medium link-primary">#TBS01</a>`);
    refreshCallbacks();
    ischeckboxcheck();
}
...
Array.prototype.slice.call(forms).forEach(function (form) {
    form.addEventListener('submit', function (event) {
        ...
        if (
            ...
            agentList.add({
                agent_id: '<a href="apps-real-estate-agent-overview.html" class="fw-medium link-primary">#TBS' + count + '</a>',
                joining_date: date,
                agent_Name: '<div class="d-flex align-items-center gap-2">\
                    <img src="'+ imageDom.src + '" alt="' + imageDom.getAttribute('alt') + '" class="avatar-xs rounded">\
                    <a href="apps-real-estate-agent-overview.html" class="text-reset text-capitalize">'+ agentNameInput.value + '</a>\
                </div>',
            ...
            Array.from(editValues).forEach(function (x) {
                ...
                if (selectedid == itemId) {
                    x.values({
                        agent_id: '<a href="apps-real-estate-agent-overview.html" class="fw-medium link-primary">' + idFieldInput.value + '</a>',
        ...
        Swal.fire({
            ...
            if (result.value) {
                for (i = 0; i < ids_array.length; i++) {
                    agentList.remove("agent_id", `<a href="apps-real-estate-agent-overview.html" class="fw-medium link-primary">${ids_array[i]}</a>`);
                }
