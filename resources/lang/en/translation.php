<?php
return [
    "menu"=>"Menu",
    "dashboards"=>"Dashboards",
    "dashboard"=>"Dashboard",
    "analytics"=>"Analytics",
    "crm"=>"CRM",
    "ecommerce"=>"Ecommerce",
    "learning" => "Learning",
    "real-estate" => 'Real Estate',
    "crypto"=>"Crypto",
    "projects"=>"Projects",
    "apps" =>"Apps",
    "calendar"=>"Calendar",
    "chat"=>"Chat",
    "mailbox"=>"Mailbox",
    "products"=>"Products",
    "product-Details"=>"Product Details",
    "product-grid" => "Product Grid",
    "create-product"=>"Add Product",
    "orders"=>"Orders",
    "order-details"=>"Order Details",
    "order-overview"=>"Order Overview",
    "customers"=>"Customers",
    "shopping-cart"=>"Shopping Cart",
    "shop-cart" => "Shop Cart",
    "checkout"=>"Checkout",
    "sellers"=>"Sellers",
    "sellers-details"=> "Seller Details",
    "sellers-overview"=> "Seller Overview",
    "list"=>"List",
    "overview"=>"Overview",
    "filemanager" => "File Manager",
    "learning" => "Learning",
    "cources" =>  "Cources",
    "my-cources" => "My Courses",
    "create-course" => "Create Course",
    "create-project"=>"Create Project",
    "custom-ui" => "Custom UI",
    "custom" => "Custom",
    "term-condition" => "Term & Conditions",
    "tasks"=>"Tasks",
    "kanbanboard"=>"Kanban Board",
    "list-view"=>"List View",
    "grid-view" => "Grid View",
    "category" => "Category",
    "overview" => "Overview",
    "create-course" => "Create Course",
    "students" => "Students",
    "subscriptions" => "My Subscriptions",
    "instructors" => "Instructors",
    "create-instructor" => "Create Instructor",
    "mycourses" => "My Courses",
    "realestate" => "Real Estate",
    "listing-grid" => "Listing Grid",
    "listing-list" => "Listing List",
    "listing-map" => "Listing Map",
    "property-overview" => "Property Overview",
    "agent" => "Agent",
    "agencies" => "Agencies",
    "agent-list" => "Agent List",
    "agent-grid" => "Agent Grid",
    "tickets-list-view" => "Tickets",
    "tickets-overview" => "Tickets Overview",
    "add-property" => "Add Property",
    "earnings" => "Earnings",
    "task-details"=>"Task Details",
    "contacts"=>"Contacts",
    "companies"=>"Companies",
    "counter" => "Counter",
    "deals"=>"Deals",
    "leads"=>"Leads",
    "transactions"=>"Transactions",
    "buy-sell"=>"Buy & Sell",
    "my-wallet"=>"My Wallet",
    "ico-list"=>"ICO List",
    "kyc-application"=>"KYC Application",
    "invoices"=>"Invoices",
    "invoice-list" => "Invoice List",
    "invoice-overview" => "Invoice Overview",
    "details"=>"Invoice Details",
    "create-invoice"=>"Create Invoice",
    "supprt-tickets"=>"Support Tickets",
    "ticket-details"=>"Ticket Details",
    "layouts"=>"Layouts",
    "horizontal"=>"Horizontal",
    "detached"=>"Detached",
    "two-column"=>"Two Column",
    "hovered"=>"Hovered",
    "vertical-hovered" => "Verical Hovered",
    "pages"=>"Pages",
    "authentication"=>"Authentication",
    "signin"=>"Sign In",
    "basic"=>"Basic",
    "cover"=>"Cover",
    "signup"=>"Sign Up",
    "password-reset"=>"Password Reset",
    "password-create"=>"Password Create",
    "lock-screen"=>"Lock Screen",
    "logout"=>"Logout",
    "success-message"=>"Success Message",
    "two-step-verification"=>"Two Step Verification",
    "errors"=>"Errors",
    "404"=>"404",
    "404-alt"=>"404 Alt",
    "500"=>"500",
    "503"=>"503",
    "404-error"=>"404 Error",
    "500-error"=>"500 Error",
    "503-error" => "503 Error",
    "offline-page"=>"Offline Page",
    "starter"=>"Starter",
    "profile"=>"Profile",
    "simple-page"=>"Simple Page",
    "settings"=>"Profile Settings",
    "team"=>"Team",
    "timeline"=>"Timeline",
    "faqs"=>"FAQs",
    "pricing"=>"Pricing",
    "gallery"=>"Gallery",
    "maintenance"=>"Maintenance",
    "coming-soon"=>"Coming Soon",
    "sitemap"=>"Sitemap",
    "search-results"=>"Search Results",
    "components"=>"Components",
    "base-ui"=>"Base UI",
    "alerts"=>"Alerts",
    "badges"=>"Badges",
    "buttons"=>"Buttons",
    "colors"=>"Colors",
    "cards"=>"Cards",
    "carousel"=>"Carousel",
    "dropdowns"=>"Dropdowns",
    "grid"=>"Grid",
    "images"=>"Images",
    "tabs"=>"Tabs",
    "accordion-collapse"=>"Accordions",
    "modals"=>"Modals",
    "offcanvas"=>"Offcanvas",
    "placeholders"=>"Placeholders",
    "progress"=>"Progress",
    "notifications"=>"Notifications",
    "media-object"=>"Media object",
    "embed-video"=>"Embed Video",
    "typography"=>"Typography",
    "lists"=>"Lists",
    "general"=>"General",
    "ribbons"=>"Ribbons",
    "utilities"=>"Utilities",
    "advance-ui"=>"Advance UI",
    "new"=>"New",
    "sweet-alerts"=>"Sweet Alerts",
    "nestable-list"=>"Nestable List",
    "scrollbar"=>"Scrollbar",
    "animation"=>"Animation",
    "tour"=>"Tour",
    "swiper-slider"=>"Swiper Slider",
    "ratings"=>"Ratings",
    "highlight"=>"Highlight",
    "scrollSpy"=>"ScrollSpy",
    "widgets"=>"Widgets",
    "forms"=>"Forms",
    "basic-elements"=>"Basic Elements",
    "form-select"=>"Form Select",
    "checkboxs-radios"=>"Checkboxs & Radios",
    "pickers"=>"Pickers",
    "input-masks"=>"Input Masks",
    "advanced"=>"Advanced",
    "range-slider"=>"Range Slider",
    "validation"=>"Validation",
    "wizard"=>"Wizard",
    "editors"=>"Editors",
    "file-uploads"=>"File Uploads",
    "form-layouts"=>"Form Layouts",
    "tables"=>"Tables",
    "basic-tables"=>"Basic Tables",
    "grid-js"=>"Grid Js",
    "list-js"=>"List Js",
    "charts"=>"Charts",
    "apexcharts"=>"Apexcharts",
    "line"=>"Line",
    "area"=>"Area",
    "column"=>"Column",
    "bar"=>"Bar",
    "mixed"=>"Mixed",
    "candlstick"=>"Candlstick",
    "boxplot"=>"Boxplot",
    "bubble"=>"Bubble",
    "scatter"=>"Scatter",
    "heatmap"=>"Heatmap",
    "treemap"=>"Treemap",
    "pie"=>"Pie",
    "radialbar"=>"Radialbar",
    "radar"=>"Radar",
    "polar-area"=>"Polar Area",
    "chartjs"=>"Chartjs",
    "echarts"=>"Echarts",
    "icons"=>"Icons",
    "remix"=>"Remix",
    "boxicons"=>"Boxicons",
    "material-design"=>"Material Design",
    "bootstrap" => "Bootstrap",
    "bootstrap-icons" => "Bootstrap Icons",
    "phosphor" => "Phosphor",
    "phosphor-icons" => "Phosphor Icons",
    "line-awesome"=>"Line Awesome",
    "feather"=>"Feather",
    "maps"=>"Maps",
    "google"=>"Google",
    "vector"=>"Vector",
    "leaflet"=>"Leaflet",
    "multi-level"=>"Multi Level",
    "level-1.1"=>"Level 1.1",
    "level-1.2"=>"Level 1.2",
    "level-2.1"=>"Level 2.1",
    "level-2.2"=>"Level 2.2",
    "level-3.1"=>"Level 3.1",
    "level-3.2"=>"Level 3.2",
    "project-list"=>"Project list",
    "Apex_Line_Chart" => "Apex Line Chart",
    "Apex_Area_Chart" => "Apex Area Chart",
    "Apex_Column_Chart" => "Apex Column Chart",
    "Apex_Bar_Chart" => "Apex Bar Chart",
    "Apex_Mixed_Chart" => "Apex Mixed Chart",
    "Apex_Timeline_Chart" => "Apex Timeline Chart",
    "Apex_Candlstick_Chart" => "Apex Candlestick Chart",
    "Apex_Boxplot_Chart" => "Apex Boxplot Chart",
    "Apex_Bubble_Chart" => "Apex Bubble Chart",
    "Apex_Scatter_Chart" => "Apex Scatter Chart",
    "Apex_Slope_Chart" => "Apex Slope Chart",
    "Apex_Heatmap_Chart" => "Apex Heatmap Chart",
    "Apex_Treemap_Chart" => "Apex Treemap Chart",
    "Apex_Pie_Chart" => "Apex Pie Chart",
    "Apex_Radialbar_Chart" => "Apex Radial Bar Chart",
    "Apex_Line_Chart" => "Apex Line Chart",
    "Apex_Radar_Chart" => "Apex Radar Chart",
    "Apex_Polar_area_Chart" => "Apex Polar Area Chart",

    "nft"=> "NFT",
    "email"=> "Email",
    "email-templates"=> "Email Templates",
    "basic-action"=> "Basic Action",
    "ecommerce-action"=> "Ecommerce Action",
    "nft-marketplace"=> "NFT Marketplace",
    "marketplace"=> "Marketplace",
    "explore-now"=> "Explore Now",
    "live-auction"=> "Live Auction",
    "item-details"=> "Item Details",
    "collections"=> "Collections",
    "creators"=> "Creators",
    "ranking"=> "Ranking",
    "wallet-connect"=> "Wallet Connect",
    "create-nft"=> "Create NFT",
    "one-page"=> "One Page",
    "nft-landing"=> "NFT Landing",
    "landing" => "Landing",
    "select2"=> "Select2",
    "datatables"=> "Datatables",
    "file-manager"=> "File Manager",
    "to-do"=> "To Do",
    "crypto-svg"=> "Crypto SVG",

    "icon-bootstrap" => "Icon Bootstrap",

    "job" => "Job",
    "jobs" => "Jobs",
    "statistics" => "Statistics",
    "job-lists" => "Job Lists",
    "candidate-lists" => "Candidate Lists",
    "grid-view" => "Grid View",
    "application" => "Application",
    "new-job" => "New Job",
    "companies-list" => "Companies List",
    "job-categories" => "Job Categories",
    "api-key" => "API Key",
    "privacy-policy" => "Privacy Policy",
    "hot" => "Hot",
    "range-area" => "Range Area",
    "funnel" => "Funnel",

    // App-specific (PROMAN)
    "blocks" => "Blocks",
    "list-blocks" => "List Blocks",
    "create-block" => "Create Block",
    "block-types" => "Block Types",

    "work-orders" => "Work Orders",
    "general-work-orders" => "General Work Orders",
    "create-work-order" => "Create Work Order",
    "block-work-orders" => "Block Work Orders",
    "create-block-work-order" => "Create Block Work Order",

    "site-visits" => "Site Visits",

    "block-inspections" => "Block Inspections",
    "list-block-inspections" => "List Block Inspections",
    "create-block-inspection" => "Create Block Inspection",
    "inspection-details" => "Inspection Details",
    "scheduled-date" => "Scheduled Date",
    "start-date" => "Start Date",
    "end-date" => "End Date",
    "lead-inspector" => "Lead Inspector",
    "inspection-team" => "Inspection Team",
    "team-members" => "Team Members",
    "inspection-assets" => "Inspection Assets",
    "schedule-inspection" => "Schedule Inspection",
    "start-inspection" => "Start Inspection",
    "complete-inspection" => "Complete Inspection",
    "inspection-status" => "Inspection Status",
    "scheduled" => "Scheduled",
    "in-progress" => "In Progress",
    "completed" => "Completed",
    "cancelled" => "Cancelled",

    "users" => "Users",
    "list-users" => "List Users",
    "create-user" => "Create User",
    "user-types" => "User Types",
    "list-user-types" => "List User Types",
    "create-user-type" => "Create User Type",
    "user-type" => "User Type",
    "user-management" => "User Management",
    "user-details" => "User Details",
    "user-type-details" => "User Type Details",
    "email-verified" => "Email Verified",
    "email-not-verified" => "Email Not Verified",
    "change-password" => "Change Password",
    "current-password" => "Current Password",
    "new-password" => "New Password",
    "confirm-password" => "Confirm Password",
    "avatar" => "Avatar",
    "upload-avatar" => "Upload Avatar",
    "remove-avatar" => "Remove Avatar",
    "no-users-found" => "No users found",
    "no-user-types-found" => "No user types found",
    "create-first-user" => "Create First User",
    "create-first-user-type" => "Create First User Type",
    "all-user-types" => "All User Types",
    "associated-users" => "Associated Users",
    "delete-user-type-confirmation" => "Are you sure you want to delete this user type? This action cannot be undone.",
    "delete-user-confirmation" => "Are you sure you want to delete this user? This action cannot be undone.",
    "edit-user-type" => "Edit User Type",
    "select-user-type" => "Select User Type",
    "new-password" => "New Password",
    "leave-blank-to-keep-current" => "Leave blank to keep current password",
    "current-avatar" => "Current Avatar",
    "edit-user" => "Edit User",
    "search-user-types" => "Search user types...",
    "search-users" => "Search users...",
    "reset" => "Reset",
    "users-count" => "Users Count",
    "dashboard" => "Dashboard",
    "id" => "ID",
    "name" => "Name",
    "email" => "Email",
    "password" => "Password",
    "created-at" => "Created At",
    "actions" => "Actions",
    "back" => "Back",
    "edit" => "Edit",
    "delete" => "Delete",
    "create" => "Create",
    "save" => "Save",
    "cancel" => "Cancel",
    "close" => "Close",
    "description" => "Description",
    "status" => "Status",
    "priority" => "Priority",
    "assigned-to" => "Assigned To",
    "reported-by" => "Reported By",
    "created" => "Created",
    "actions" => "Actions",
    "category" => "Category",
    "issue-category" => "Category",
    "select-issue-category" => "Select Category",
    "problem-overview" => "Problem Overview",
    "search-by-problem-overview" => "Search by problem overview, description, or reference number...",
    "issues" => "Issues",
    "block-issues" => "Block Issues",
    "create-block-issue" => "Create Block Issue",
    "general-issues" => "General Issues",
    "create-issue" => "Create Issue",

    // Block Issues (list)
    "block-issues-management" => "Block Issues Management",
    "create-block-issue-btn" => "Create Block Issue",
    "search-issues" => "Search issues...",
    "all-blocks" => "All Blocks",
    "all-status" => "All Status",
    "open" => "Open",
    "in-progress" => "In Progress",
    "resolved" => "Resolved",
    "closed" => "Closed",
    "on-hold" => "On Hold",
    "all-priority" => "All Priority",
    "low" => "Low",
    "normal" => "Normal",
    "high" => "High",
    "urgent" => "Urgent",
    "critical" => "Critical",
    "search" => "Search",
    "clear-filters" => "Clear Filters",
    "reference" => "Reference",
    "title" => "Title",
    "block" => "Block",
    "priority-label" => "Priority",
    "status-label" => "Status",
    "assigned-to" => "Assigned To",
    "reported-by" => "Reported By",
    "created" => "Created",
    "actions" => "Actions",
    "view" => "View",
    "edit" => "Edit",
    "delete" => "Delete",
    "no-block-issues-found" => "No block issues found",
    "create-first-issue" => "Create First Issue",
    "confirm-deletion" => "Confirm Deletion",
    "confirm-deletion-text" => "Are you sure you want to delete issue :ref? This action cannot be undone.",
    "cancel" => "Cancel",
    "delete-verb" => "Delete",
];
?>

