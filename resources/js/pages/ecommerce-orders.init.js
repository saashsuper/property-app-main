...
xhttp.onload = function () {
    ...
    Array.from(json_records).forEach(function (element) {
        ...
        orderList.add({
            order_id: `<a href="apps-ecommerce-order-overview.html" class="fw-medium link-primary">#TBS2500${element.id}</a>`,
    ...
    orderList.remove("order_id", `<a href="apps-ecommerce-order-overview.html" class="fw-medium link-primary">#TBS250001</a>`);
    refreshCallbacks();
    ischeckboxcheck();
}
...
Array.prototype.slice.call(forms).forEach(function (form) {
    form.addEventListener('submit', function (event) {
        ...
        if (customerNameField.value !== "" &&
            ...
            orderList.add({
                order_id: '<a href="apps-ecommerce-order-overview.html" class="fw-medium link-primary">#TBS2500' + count + '</a>',
            ...
            Array.from(editValues).forEach(function (x) {
                ...
                if (selectedid == itemId) {
                    x.values({
                        order_id: '<a href="apps-ecommerce-order-overview.html" class="fw-medium link-primary">' + idField.value + '</a>',
                        order_date: orderDateField.value,
        ...
        Swal.fire({
            ...
            if (result.value) {
                for (i = 0; i < ids_array.length; i++) {
                    orderList.remove("order_id", `<a href="apps-ecommerce-order-overview.html" class="fw-medium link-primary">${ids_array[i]}</a>`);
                }
