...
function loadMailData(datas) {
    ...
    Array.from(datas).forEach(function (mailData, index) {
        ...
        <div class="col-mail col-mail-1">\
            ...
            <button type="button" class="btn avatar-xs p-0 favorite-btn fs-15 ' + checkStarred + '">\
...
function favoriteBtn() {
    Array.from(document.querySelectorAll(".favorite-btn")).forEach(function (item) {
...
Array.from(document.querySelectorAll('.toggle-email-panel')).forEach(function (item) {
    item.addEventListener('click', function () {
        if (!document.querySelector('.email-wrapper .email-panel').classList.contains('show')) {
            document.querySelector('.email-wrapper .email-panel').classList.add('show');
        } else {
            document.querySelector('.email-wrapper .email-panel').classList.remove('show');
        }
    });
});

Array.from(document.querySelectorAll(".mail-user-dropdown")).forEach(function (elem) {
    elem.querySelectorAll(".dropdown-menu .dropdown-item").forEach(function (item) {
        item.addEventListener('click', function () {
            elem.querySelector('.mail-profile-img').src = item.querySelector('.mail-user-img').src;
        });
    });
});
...
searchList.addEventListener("keyup", function () {
    ...
    function filterItems(arr, query) {
        return arr.filter(function (el) {
            return el.name.toLowerCase().indexOf(query.toLowerCase()) !== -1 || el.description.toLowerCase().indexOf(query.toLowerCase()) !== -1 || el.title.toLowerCase().indexOf(query.toLowerCase()) !== -1
        })
    }
